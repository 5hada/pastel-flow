...
- 사용자는 도구 페이지에서 Tool Module 루트 폴더를 등록하고, `tool-schema.md` 규격에 맞는 모듈을 검증, 관리, 단독 실행할 수 있다.
...
- 우측 작업 영역에는 좌측과 동일한 Tool Module 목록을 다시 표시하지 않는다.
- 좌측 패널의 목록은 등록된 Tool Module 루트 폴더 내부의 실제 계층 구조를 그대로 반영한다.
- 우측 작업 영역: 선택된 Tool Module의 상세, 검증 결과, 단독 실행 UI, Workflow 추가 액션을 표시한다.
- 선택된 Tool Module이 없으면 루트 폴더 등록 진입과 빈 상태를 표시한다.

Tool Module 등록 방식:

- 개별 Tool Module 폴더를 하나씩 등록하지 않는다.
- 사용자는 Tool Module들을 저장할 루트 폴더 하나를 등록한다.
- Pastel Flow는 해당 루트 폴더 아래의 계층 구조를 스캔해 `tool-schema.md` 규격을 만족하는 모듈을 인식한다.
- 좌측 패널은 루트 폴더 내부의 폴더 계층을 그대로 사용해 Tool Module을 표시하고 관리한다.
- 루트 폴더 안에서 규격을 만족하지 않는 폴더는 검증 실패 또는 무시 상태로 표시한다.
...
- Tool Module 루트 폴더가 없으면 도구 페이지에서 루트 폴더 등록 진입을 제공한다.
- Tool Module 루트 폴더는 있지만 인식된 모듈이 없으면 우측 작업 영역에 모듈 없음 상태와 폴더 구조 확인 안내를 표시한다.
...
- 전체 창에는 별도 가로/세로 스크롤바가 없어야 하며, 스크롤은 좌측 패널과 우측 작업 영역의 세로 스크롤만 사용한다.
- 좌측 패널 토글 버튼은 패널 열림/닫힘 상태와 무관하게 좌측 끝의 동일한 위치를 유지한다.
